...
<div class="modal-header">
    ...
    <h2>
      <a href="<?php echo gameLink($serverIp, $server['serverPort']) ?>"><?php echo $server['serverName'] ?></a> 
    </h2>
    <a id="mo-refresh-button" 
       class="btn btn-info" 
       href="#" 
       title="Refresh server info."
       onclick="getServerInfo(this, '<?php echo $serverIp . "','" . $server['serverPort'] ?>')"><i class="icon-refresh icon-white"></i></a>
    <a id="mo-pounce-button" 
       class="btn btn-info" 
       href="#" 
       title="Pounce! Try to join the server as soon as there is a free slot."
       data-waiting-text="waiting..." 
       onclick="pounce(this, '<?php echo $serverIp . "','" . $server['serverPort'] ?>')"><i class="icon-time icon-white"></i></a>
  </div>
  ...
  <div class="modal-body">
    ...
    <div class="server-players">
      <p class="lead">Current Players:</p>
      <?php if(isset($server['playerList'])):?>
      <ul>
        <?php foreach($server['playerList'] as $player):?>
  ...
  <div class="modal-footer">
    ...
    <a href="<?php echo gameLink($serverIp, $server['serverPort']) ?>" class="btn btn-success"><i class="icon-play icon-white"></i> Play</a> 
  </div>
<!--
